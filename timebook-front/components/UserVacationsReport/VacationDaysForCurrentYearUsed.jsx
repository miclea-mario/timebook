...
const VacationDaysForCurrentYearUsed = ({ id, currentYear, getTotal }) => {
    ...
    <>
      {status === 'loading' && (
        <h3 className="mt-4 font-bold flex items-center">
          Zile de concediu folosite pentru {currentYear}:{' '}
      ...
      {status === 'success' && (
        <h3 className="mt-4 font-bold">
          Zile de concediu folosite pentru {currentYear}:{' '}
          <Plural count={getTotal(data) * -1} one="zi" many="zile" />
        </h3>
      )}
    </>
  );
...
export default VacationDaysForCurrentYearUsed;
